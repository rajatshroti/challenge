...
public class AccountsControllerTest {
  ...
  public void transferMoney() throws Exception {
	    ...
	    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountFromId\":\"Id-101\",\"accountToId\":\"Id-102\",\"amount\":125.45}")).andExpect(status().isAccepted());
  ...
  public void getAccountAfterCredit() throws Exception {
	  String uniqueAccountIdFrom= "Id-101";
	    ...
	    this.mockMvc.perform(get("/v1/accounts/" + uniqueAccountIdTo))
	      .andExpect(status().isOk())
	      .andExpect(
	        content().string("{\"accountId\":\"" + uniqueAccountIdTo + "\",\"balance\":348.75}"));
	  }
  @Test
  public void getAccountAfterDebit() throws Exception {
	  String uniqueAccountIdFrom= "Id-101";
	    ...
	    BalanceTransferRequest balanceTransferRequest= new BalanceTransferRequest(uniqueAccountIdFrom, uniqueAccountIdTo, new BigDecimal("125.45"));
	    this.accountsService.transferMoney(balanceTransferRequest);
	    this.mockMvc.perform(get("/v1/accounts/" + uniqueAccountIdFrom))
	      .andExpect(status().isOk())
	      .andExpect(
	        content().string("{\"accountId\":\"" + uniqueAccountIdFrom + "\",\"balance\":400.00}"));
	  }
  @Test
  public void transferMoneyToWrongAccountTo() throws Exception {
	    ...
	    this.accountsService.createAccount(accountFrom);
	    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountFromId\":\"Id-101\",\"accountToId\":\"Id-103\",\"amount\":125.45}")).andExpect(status().isBadRequest()).andExpect(content().string("Id-103 account does not exists!"));
  ...
  public void transferMoneyToWrongAccountFrom() throws Exception {
	    ...
	    this.accountsService.createAccount(accountTo);
	    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountFromId\":\"Id-103\",\"accountToId\":\"Id-101\",\"amount\":125.45}")).andExpect(status().isBadRequest()).andExpect(content().string("Id-103 account does not exists!"));
  ...
  public void transferMoneyToSameAccount() throws Exception {
	    ...
	    this.accountsService.createAccount(accountTo);
	    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountFromId\":\"Id-101\",\"accountToId\":\"Id-101\",\"amount\":125.45}")).andExpect(status().isBadRequest()).andExpect(content().string("From and To accounts are same."));
  ...
  public void transferMoneyGreaterThenBalance() throws Exception {
	    ...
	    this.accountsService.createAccount(accountTo);
	    this.mockMvc.perform(post("/v1/accounts/transferMoney").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountFromId\":\"Id-101\",\"accountToId\":\"Id-102\",\"amount\":725.45}")).andExpect(status().isBadRequest()).andExpect(content().string("Your account does not have sufficent balance."));
